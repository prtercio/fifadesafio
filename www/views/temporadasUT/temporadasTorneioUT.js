( function() {
	'use strict';
	var copasUT = angular.module( 'App.CtrlTempTorneioUT', [] );
	copasUT.controller( 'CtrlTempTorneioUT', [ '$scope', 'idTorneio', '$localStorage', '$state', '$ionicPopup', '$window', '$ionicLoading', 'dataService',
		function( $scope, idTorneio, $localStorage, $state, $ionicPopup, $window, $ionicLoading, dataService ) {
			...
			*/
			...
			var ranking = [];
			var ref = firebase.database().ref( 'desafio/desafios/temporadasUT/oficial/' + idTorneio );
			...
			$scope.entrarTorneio = function() {
				...
				if ( $scope.loggedIn ) {
					...
					firebase.database().ref().child( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/inscritos/' ).push( {
						key: keyUser,
						gamertag: gamertag,
						jogos: jsondJogos,
						vitoria: 0,
						...
						firebase.database().ref().child( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/configuracao' ).update( {
	    ...
	    */
			$scope.sairTorneio = function() {
				...
				function confirmarSaida() {
					$ionicLoading.show( {} );
					//if ( res ) {
					firebase.database().ref( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/inscritos/' + keyUserInscritoTorneio ).remove();
					firebase.database().ref( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/configuracao' ).update( {
						inscritos: gamesInscritos - 1
					} ).then( function( response ) {
			...
			function cargarJogos() {
				...
				var refJogos = firebase.database().ref( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/inscritos/' + keyUserInscritoTorneio );
