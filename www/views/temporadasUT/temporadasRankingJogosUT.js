( function() {
  'use strict';
  var temporadasRankingJogosUT = angular.module( 'App.CtrlTemporadasRankingJogosUT', [] );
  temporadasRankingJogosUT.controller( 'CtrlTemporadasRankingJogosUT', [ '$scope', 'Utils', '$state', '$localStorage', 'Popup', '$stateParams', '$window', 'idJogos', '$ionicNavBarDelegate', 'dataService',
    function( $scope, Utils, $state, $localStorage, Popup, $stateParams, $window, idJogos, $ionicNavBarDelegate, dataService ) {
      ...
      var resultadoJogos = [];
      var refjogos = firebase.database().ref( 'desafio/desafios/temporadasUT/oficial/' + idTorneio + '/inscritos/' + keyUsuario ).orderByChild( 'jogo' );
