( function() {
  'use strict';
  var temporadasRankingJogosChampions = angular.module( 'App.CtrlTemporadasRankingJogosChampions', [] );
  temporadasRankingJogosChampions.controller( 'CtrlTemporadasRankingJogosChampions', [ '$scope', 'Utils', '$state', '$localStorage', 'Popup', '$stateParams', '$window', 'idJogos', '$ionicNavBarDelegate', 'dataService',
    function( $scope, Utils, $state, $localStorage, Popup, $stateParams, $window, idJogos, $ionicNavBarDelegate, dataService ) {
      ...
      var resultadoJogos = [];
      var refjogos = firebase.database().ref( 'desafio/desafios/temporadasChampions/oficial/' + idTorneio + '/inscritos/' + keyUsuario ).orderByChild( 'jogo' );
